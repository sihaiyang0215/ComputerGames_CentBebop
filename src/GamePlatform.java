...
public class GamePlatform extends BufferedApplet {
	...
	/**
	 * Callback function, invoked at the start of a collision between two
	 * pieces.
	 */
	...
	public void render(Graphics g) {
		if (w == 0) {
			...
			h = bounds().height;
			setup();
			isColliding = new boolean[collisionTrackedPieces.size()][pieces
					.size()];
			...
			for (int j = 0; j < pieces.size(); j++) {
				...
				if (piece1 != piece2) {
					...
					if (collision && !isColliding[i][j]) {
						...
						if (verbosity >= 1)
							eventBuffer.append(" START_COLLISION { "
									+ piece1.getId() + " " + piece2.getId()
									+ " }");
					}
						...
						if (verbosity >= 1)
							eventBuffer.append(" END_COLLISION { "
									+ piece1.getId() + " " + piece2.getId()
									+ " }");
					}
...
//		}
	...
	/***************************************************************************************************
	 * Strategy for tracking objects: -- instantiate a shadow fields: an array
	 * of field valuess corresponding to the ones in the original object. -- at
	 * each update: -- compare object's field values with corresponding shadow
	 * field values. -- if any field values have changed: -- write out
	 * name/value pairs of all changed fields of the object. -- for all changed
	 * fields: set shadow field values to the object's field values.
	 */
	...
	public void track(Object object, String name, String varNames) {
		...
		for (StringTokenizer st = new StringTokenizer(varNames); st
				.hasMoreTokens();)
